...
2. 按 BOARD 取下一个 `todo` 任务(当前优先:**<填:第一个任务>**)。读对应 `dev/research/findings/` 设计的接线 + 对抗测试要点。
...
5. 红线见 `RULES.md`(通用) + `RULES.project.md`(本项目);不擅自 commit;致命错误即停工。只有遇到 `DECISIONS.md` 没覆盖的新岔路才停下来问用户。
